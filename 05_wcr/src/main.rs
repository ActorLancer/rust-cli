...
fn format_field(value: usize, show: bool) -> String {
    if show {
        // 右对齐：> 表示内容在8个字符宽度的空间内右对齐，宽度为8,空格补齐
        format!("{value:>8}")
    } else {
        "".to_string()
    }
...
mod tests {
    ...
    // 允许把一个内存缓冲区（如 Vec<u8> 或 &[u8]）当作一个“文件”一样来读写和定位（seek）
    // 本质：把内存中的数据包包装成一个实现了 Read、Write、Seek 等 trait 的对象
    // 用途：可以用来模拟文件操作，但数据其实都在内存里，非常适合测试或临时数据处理
    use std::io::Cursor;
